> 重构后所有命令从项目根目录运行，使用 `python -m backend.src.algorithms.blink_detection.模块名`。安装与测试见 [开发文档](../../../../docs/development.md)。
...
本模块严格遵循 `docs/specifications/blink-detection.md` 实现。
...
```bash
pip install -r backend/src/algorithms/blink_detection/requirements.txt
```
...
```bash
cd blink_detection
python -m backend.src.algorithms.blink_detection.main
```

### 4.2 摄像头 + 调试 overlay + 保存 CSV

```bash
python -m backend.src.algorithms.blink_detection.main --debug --record
```

### 4.3 视频文件

```bash
python -m backend.src.algorithms.blink_detection.main --source demo.mp4 --debug
```

### 4.4 robust 模式

```bash
python -m backend.src.algorithms.blink_detection.main --mode robust --debug
```
...
```bash
cd blink_detection
python -m unittest backend.tests.unit.test_blink_detector -v
```
