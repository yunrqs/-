> 重构后所有命令从项目根目录运行，使用 `python -m backend.src.algorithms.emotion.模块名`。安装与测试见 [开发文档](../../../../docs/development.md)。

📁 项目结构总览
text

backend/src/algorithms/emotion/
...
1. train_with_fer2013.py — FER-2013 CSV 数据集训练
   ...
   使用类别权重处理数据不平衡问题
   支持数据增强（随机翻转、旋转）
   自动保存最佳模型到 backend/models/emotion/fer2013_best_model.pth
...
├── surprise/
└── neutral/
输出：保存最佳模型到 backend/models/emotion/best_model.pth 3. use_model.py — 实时摄像头情绪识别（基础版）
功能：调用摄像头进行实时人脸情绪识别
依赖模型：backend/models/emotion/fer2013_best_model.pth（由 train_with_fer2013.py 生成）
特点：
...
输出：保存到 backend/models/emotion/fer2013_resnet50.pth
🚀 使用说明
环境准备
确保安装以下依赖：
...
准备数据：将 FER-2013 数据集拆分为 train.csv、val.csv、test.csv，放入 data/raw/fer2013/ 目录
训练模型：
bash

cd emo/emotion
python -m backend.src.algorithms.emotion.train_with_fer2013
实时识别：
bash
...
python -m backend.src.algorithms.emotion.train_model
实时识别：
bash

python -m backend.src.algorithms.emotion.emotion_predictor
方案三：单张图片/视频识别
bash

# 单张图片（需修改代码中的路径）

python -m backend.src.algorithms.emotion.predict_image

# 视频文件（需修改代码中的路径）

python -m backend.src.algorithms.emotion.predict_video
方案四：使用预训练模型（无需训练）
bash

# 下载预训练模型

python -m backend.src.algorithms.emotion.download_pretrained_model

# 或在代码中直接使用 pretrained_emotion.py

python -m backend.src.algorithms.emotion.pretrained_emotion
⚠️ 注意事项
问题 解决方案
模型路径不匹配 检查 use_model.py 中的 model_path 是否指向正确位置
找不到 CSV 文件 确保 data/raw/fer2013/ 目录下有 train.csv、val.csv
